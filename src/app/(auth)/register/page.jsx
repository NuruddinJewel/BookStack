...
import Link from "next/link";
import { useRouter } from "next/navigation";
import { FiUser, FiMail, FiLock, FiEye, FiEyeOff, FiAlertCircle, FiBookOpen, FiEdit3 } from "react-icons/fi";
import { FcGoogle } from "react-icons/fc";
import { signUp, signIn } from "@/lib/auth-client";
import { toast } from "react-toastify";
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const { error: authError } = await signUp.email({
                ...
                email: form.email,
                password: form.password,
                role: selectedRole, // "user" or "writer"
            });
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <button
                                        ...
                                        className={`flex flex-col items-center gap-2 rounded-lg border px-3 py-4 transition-colors
                                            ${selectedRole === "user"
                                                ...
                                                : "border-[var(--border)] bg-[var(--cream-2)] hover:border-[var(--ink-3)]"
                                            }`}
                                    >
                                        <FiBookOpen
                                            size={20}
                                            className={selectedRole === "user" ? "text-[var(--amber)]" : "text-[var(--ink-3)]"}
                                        />
                                        <span
                                            className={`text-sm font-medium ${selectedRole === "user" ? "text-[var(--amber)]" : "text-[var(--ink-2)]"
                                                }`}
                                        >
                                            Reader
                                        </span>
                                        <span className="text-[11px] text-[var(--ink-3)] text-center leading-tight">
                                            Browse &amp; buy ebooks
                                        </span>
                                    </button>
                                    ...
                                    >
                                        <FiEdit3
                                            size={20}
                                            className={selectedRole === "writer" ? "text-[var(--amber)]" : "text-[var(--ink-3)]"}
                                        />
                                        <span
                                            className={`text-sm font-medium ${selectedRole === "writer" ? "text-[var(--amber)]" : "text-[var(--ink-2)]"
                                                }`}
                                        >
                                            Writer
                                        </span>
