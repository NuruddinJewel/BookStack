    ...
    useEffect(() => {
        ...
        async function fetchHistory() {
            ...
            try {
                ...
                const res = await fetch(`${apiUrl}/user/history?userId=${userId}`);
                            ...
                            <tbody className="divide-y divide-[var(--border)]">
                                {history.map((tx) => (
                                    <tr key={tx._id} className="hover:bg-[rgba(0,0,0,0.01)] transition-colors">
                                        <td className="p-4 text-xs font-mono text-[var(--ink-3)]">
                                            {tx.stripeSessionId ? tx.stripeSessionId.substring(0, 14) : tx._id?.substring(0, 8)}...
                                        </td>
                                        <td className="p-4 text-sm font-medium text-[var(--ink)]">
                                            {tx.ebookTitle || "Premium Ebook"}
                                        </td>
                                        <td className="p-4 text-sm text-[var(--ink-2)]">
                                            {new Date(tx.purchaseDate).toLocaleDateString('en-US', {
                                                year: 'numeric',
