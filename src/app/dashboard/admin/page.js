...
export default function AdminPage() {
    ...
    const [totals, setTotals] = useState({ totalUsers: 0, totalWriters: 0, totalEbooks: 0, totalSold: 0, totalRevenue: 0 });
    ...
    useEffect(() => {
        ...
        async function fetchAnalytics() {
            try {
                ...
                const res = await fetch(`${apiUrl}/admin/analytics`, { signal: controller.signal });

                if (res.ok) {
                    const data = await res.json();
                    setChartData({ sales: data?.sales || [], genres: data?.genres || [] });
        ...
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                <div className="p-5 bg-[var(--cream-2)] border border-[var(--border)] rounded-xl">
                    ...
                    <h3 className="text-2xl font-bold mt-1.5 text-[var(--ink)] m-0">{totals.totalUsers || 0}</h3>
                </div>
                <div className="p-5 bg-[var(--cream-2)] border border-[var(--border)] rounded-xl">
                    <p className="text-xs font-semibold uppercase tracking-wider text-[var(--ink-3)] m-0">Total Writers</p>
                    <h3 className="text-2xl font-bold mt-1.5 text-[var(--ink)] m-0">{totals.totalWriters || 0}</h3>
                </div>
                <div className="p-5 bg-[var(--cream-2)] border border-[var(--border)] rounded-xl">
                    <p className="text-xs font-semibold uppercase tracking-wider text-[var(--ink-3)] m-0">Total Ebooks</p>
                    <h3 className="text-2xl font-bold mt-1.5 text-[var(--ink)] m-0">{totals.totalEbooks || 0}</h3>
                </div>
                <div className="p-5 bg-[var(--cream-2)] border border-[var(--border)] rounded-xl">
                    <p className="text-xs font-semibold uppercase tracking-wider text-[var(--ink-3)] m-0">Total Revenue</p>
                    <h3 className="text-2xl font-bold mt-1.5 text-[var(--ink)] m-0">${(totals.totalRevenue || 0).toFixed(2)}</h3>
                </div>
            </div>
