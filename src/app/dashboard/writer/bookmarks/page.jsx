...
export default function WriterBookmarkPage() {
    ...
    const fetchBookmarks = async (writerId, signal) => {
        try {
            const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000';
            const res = await fetch(`${apiUrl}/writer/bookmarks?writerId=${writerId}`, { signal });
    ...
    useEffect(() => {
        ...
        const controller = new AbortController();
        fetchBookmarks(writerId, controller.signal);
    ...
    const handleRemoveBookmark = async (bookmarkId) => {
        try {
            const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000';
            const res = await fetch(`${apiUrl}/writer/bookmarks/${bookmarkId}`, { method: 'DELETE' });
